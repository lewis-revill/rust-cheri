    ...
    fn codegen_return_terminator(&mut self, mut bx: Bx) {
        ...
        let llval = match &self.fn_abi.ret.mode {
            ...
            PassMode::Cast(cast_ty, _) => {
                ...
                let addr = bx.pointercast(llslot, bx.type_ptr_to_ext(ty, bx.tcx().data_layout.default_address_space));
    ...
    fn codegen_drop_terminator(
        ...
        let (drop_fn, fn_abi) = match ty.kind() {
            ...
            ty::Dynamic(_, _, ty::DynStar) => {
                ...
                let vtable = bx.load(bx.type_i8p_ext(dl.instruction_address_space), vtable_ptr, abi::Align::ONE);
                // Truncate vtable off of args list
                args = &args[..1];
                debug!("args' = {:?}", args);
                (
            ...
            Some(intrinsic) => {
                let dest = match ret_dest {
                    ...
                    ReturnDest::Nothing => {
                        ...
                        bx.const_undef(bx.type_ptr_to_ext(bx.arg_memory_ty(&fn_abi.ret), bx.tcx().data_layout.default_address_space))
                    }
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn codegen_argument(
        ...
        if by_ref && !arg.is_indirect() {
            ...
            if let PassMode::Cast(ty, _) = &arg.mode {
                ...
                let addr = bx.pointercast(llval, bx.type_ptr_to_ext(llty, bx.tcx().data_layout.default_address_space));
    ...
    fn codegen_transmute_into(
        ...
        let cast_ptr = bx.pointercast(dst.llval, bx.type_ptr_to_ext(llty, bx.tcx().data_layout.default_address_space));
