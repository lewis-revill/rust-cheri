...
impl<CTX> crate::ty::HashStable<CTX> for ScalarInt {
    fn hash_stable(&self, hcx: &mut CTX, hasher: &mut crate::ty::StableHasher) {
        ...
        self.range.get().hash_stable(hcx, hasher);
...
impl<D: Decoder> Decodable<D> for ScalarInt {
    fn decode(d: &mut D) -> ScalarInt {
        ScalarInt { data: d.read_u128(), range: NonZeroU8::new(d.read_u8()).unwrap(), size: NonZeroU8::new(d.read_u8()).unwrap() }
    }
}

impl ScalarInt {
    pub const TRUE: ScalarInt = ScalarInt { data: 1_u128, range: NonZeroU8::new(1).unwrap(), size: NonZeroU8::new(1).unwrap() };

    pub const FALSE: ScalarInt = ScalarInt { data: 0_u128, range: NonZeroU8::new(1).unwrap(), size: NonZeroU8::new(1).unwrap() };
    ...
    pub fn range(self) -> Size {
        Size::from_bytes(self.range.get())
    }
    ...
    pub fn null(size: Size) -> Self {
        // FIXME: Temporary.
        Self { data: 0, range: NonZeroU8::new(size.bytes() as u8).unwrap(), size: NonZeroU8::new(size.bytes() as u8).unwrap() }
    }
    ...
    pub fn try_from_uint(i: impl Into<u128>, size: Size) -> Option<Self> {
        ...
        if size.truncate(data) == data {
            // FIXME: Temporary.
            Some(Self { data, range: NonZeroU8::new(size.bytes() as u8).unwrap(), size: NonZeroU8::new(size.bytes() as u8).unwrap() })
        } else {
            None
        }
    }
    ...
    #[inline]
    pub fn try_from_uint_r(i: impl Into<u128>, range: Size, size: Size) -> Option<Self> {
        let data = i.into();
        if size.truncate(data) == data {
            // FIXME: Temporary.
            Some(Self { data, range: NonZeroU8::new(range.bytes() as u8).unwrap(), size: NonZeroU8::new(size.bytes() as u8).unwrap() })
        } else {
            None
        }
    }
    ...
    pub fn try_from_int(i: impl Into<i128>, size: Size) -> Option<Self> {
        ...
        if size.sign_extend(truncated) as i128 == i {
            // FIXME: Temporary.
            Some(Self { data: truncated, range: NonZeroU8::new(size.bytes() as u8).unwrap(), size: NonZeroU8::new(size.bytes() as u8).unwrap() })
        } else {
            None
        }
    }
    ...
    pub fn to_bits(self, target_size: Size) -> Result<u128, Size> {
        ...
        if target_size.bytes() == u64::from(self.range.get()) {
            self.check_data();
            Ok(self.data)
        } else {
            Err(self.range())
    ...
    pub fn try_to_machine_usize<'tcx>(&self, tcx: TyCtxt<'tcx>) -> Result<u64, Size> {
        ...
        Ok(self.to_bits(tcx.data_layout.ptr_layout(None).idx_size)? as u64)
    }
        ...
        $(
            impl From<$ty> for ScalarInt {
                ...
                fn from(u: $ty) -> Self {
                    Self {
                        data: u128::from(u),
                        range: NonZeroU8::new(std::mem::size_of::<$ty>() as u8).unwrap(),
...
impl From<char> for ScalarInt {
    #[inline]
    fn from(c: char) -> Self {
        Self { data: c as u128, range: NonZeroU8::new(std::mem::size_of::<char>() as u8).unwrap(), size: NonZeroU8::new(std::mem::size_of::<char>() as u8).unwrap() }
    }
}
...
impl From<Single> for ScalarInt {
    ...
    fn from(f: Single) -> Self {
        ...
        Self { data: f.to_bits(), range: NonZeroU8::new((Single::BITS / 8) as u8).unwrap(), size: NonZeroU8::new((Single::BITS / 8) as u8).unwrap() }
    }
}
...
impl From<Double> for ScalarInt {
    ...
    fn from(f: Double) -> Self {
        // We trust apfloat to give us properly truncated data.
        Self { data: f.to_bits(), range: NonZeroU8::new((Double::BITS / 8) as u8).unwrap(), size: NonZeroU8::new((Double::BITS / 8) as u8).unwrap() }
    }
}
