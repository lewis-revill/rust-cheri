        ...
        match *ty.kind() {
            ...
            ty::Array(ty, sz) => {
                ...
                if self.should_print_verbose() {
                    ...
                    p!("_");
                } else if let Some(n) = sz.kind().try_to_bits(self.tcx().data_layout.ptr_layout(None).idx_size) {
                    p!(write("{}", n));
    ...
    fn pretty_print_const_scalar_int(
        ...
        match ty.kind() {
            ...
            ty::Ref(..) | ty::RawPtr(_) | ty::FnPtr(_) => {
                // TODO: More complexity needed here.
                let data = int.assert_bits(self.tcx().data_layout.ptr_layout(None).idx_size);
                self = self.typed_value(
                    |mut this| {
