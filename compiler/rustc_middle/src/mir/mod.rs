    ...
    ty::tls::with(|tcx| {
        ...
        match (ct, ty.kind()) {
            ...
            (ConstValue::ByRef { alloc, offset }, ty::Array(t, n)) if *t == u8_type => {
                ...
                let n = n.kind().try_to_bits(tcx.data_layout.ptr_layout(None).idx_size).unwrap();
                // cast is ok because we already checked for pointer size (32 or 64 bit) above
                let range = AllocRange { start: offset, size: Size::from_bytes(n), range: Size::from_bytes(n) };
