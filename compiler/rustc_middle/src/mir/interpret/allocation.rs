...
impl Allocation {
    ...
    pub fn adjust_from_tcx<Prov, Extra, Err>(
        ...
        // TODO: More complexity needed here. Unsure if this should be idx_size or ty_size.
        let ptr_size = cx.data_layout().ptr_layout(None).idx_size.bytes_usize();
...
impl<Prov: Copy, Extra> Allocation<Prov, Extra> {
    ...
    fn range_get_provenance(&self, cx: &impl HasDataLayout, range: AllocRange) -> &[(Size, Prov)] {
        ...
        // TODO: More complexity needed here. idx_size vs ty_size.
        let start = range.start.bytes().saturating_sub(cx.data_layout().ptr_layout(None).idx_size.bytes() - 1);
        ...
        Prov: Provenance,
    {
        ...
        let (first, last) = {
            ...
            (
                ...
                // TODO: More complexity needed here. idx_size vs ty_size.
                provenance.last().unwrap().0 + cx.data_layout().ptr_layout(None).idx_size,
            )
        };
        ...
        if last > end {
            if Prov::ERR_ON_PARTIAL_PTR_OVERWRITE {
                return Err(AllocError::PartialPointerOverwrite(
                    // TODO: More complexity needed here. idx_size vs ty_size.
                    last - cx.data_layout().ptr_layout(None).idx_size,
                ));
            }
            warn!(
