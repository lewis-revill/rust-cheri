...
impl Default for TargetDataLayout {
    ...
    fn default() -> TargetDataLayout {
        ...
        TargetDataLayout {
            ...
            pointer_layouts: vec![
                (AddressSpace::DATA, PointerLayout::default()),
            ],
...
impl TargetDataLayout {
    pub fn parse<'a>(target: &'a Target) -> Result<TargetDataLayout, TargetDataLayoutErrors<'a>> {
        ...
        let align_noslice = |aa: &'a str, pa: &'a str, cause: &'a str| -> Result<AbiAndPrefAlign, TargetDataLayoutErrors<'_>> {
            let align_from_bits = |bits| {
                Align::from_bits(bits)
                    .map_err(|err| TargetDataLayoutErrors::InvalidAlignment { cause, err })
            };
        ...
        for spec in target.data_layout.split('-') {
            ...
            match &*spec_parts {
                ...
                [p, s, aa, pa, i, ..] if p.starts_with('p') && i.parse::<u64>().is_ok() && pa.parse::<u64>().is_ok() => {
                    let mut pl = PointerLayout::default();
                    pl.ty_size = size(s, p)?;
                    pl.idx_size = size(i, p)?;
                    pl.is_fat_ty = p.len() > 1 && p.chars().nth(1).unwrap() == 'f';
                    pl.align = align_noslice(aa, pa, p)?;
                    let as_field = &p[(if pl.is_fat_ty { 2 } else { 1 })..];
                    let address_space = if as_field.len() > 0 { parse_address_space(as_field, p)? } else { AddressSpace(0) };
        ...
        let dl_pointer_size = dl.ptr_layout(None).idx_size.bits();
    ...
    pub fn obj_size_bound(&self, address_space: Option<AddressSpace>) -> u64 {
        match self.ptr_layout(address_space).idx_size.bits() {
            16 => 1 << 15,
            32 => 1 << 31,
            64 => 1 << 47,
    ...
    pub fn ptr_sized_integer(&self, address_space: Option<AddressSpace>) -> Integer {
        match self.ptr_layout(address_space).idx_size.bits() {
            ...
            32 => I32,
            64 => I64,
            bits => panic!("idx_sized_integer: unknown pointer idx size {}", bits),
        }
    }
