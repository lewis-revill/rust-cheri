        ...
        let llval = match name {
            ...
            sym::volatile_load | sym::unaligned_volatile_load => {
                ...
                let load = if let PassMode::Cast(ty, _) = &fn_abi.ret.mode {
                    ...
                    let ptr = self.pointercast(ptr, self.type_ptr_to_ext(llty, dl.default_address_space));
            ...
            sym::raw_eq => {
                ...
                let use_integer_compare = match layout.abi() {
                    ...
                    Aggregate { .. } => {
                        ...
                        layout.size() <= dl
                            .ptr_layout(Some(dl.alloca_address_space)).idx_size * 2
                    }
                };
...
fn try_intrinsic<'ll>(
    ...
    if bx.sess().panic_strategy() == PanicStrategy::Abort {
        // TODO: Get the correct address space.
        let try_func_ty = bx.type_func(&[bx.type_i8p_ext(bx.tcx().data_layout.default_address_space)], bx.type_void());
...
fn codegen_msvc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let try_func_ty = bx.type_func(&[bx.type_i8p_ext(dl.default_address_space)], bx.type_void());
        ...
        let type_info_vtable = bx.declare_global("??_7type_info@@6B@", bx.type_i8p_ext(dl.globals_address_space));
        let type_name = bx.const_bytes(b"rust_panic\0");
        let type_info =
            bx.const_struct(&[type_info_vtable, bx.const_null(bx.type_i8p_ext(dl.globals_address_space)), type_name], false);
        ...
        let catch_ty = bx.type_func(&[bx.type_i8p_ext(dl.default_address_space), bx.type_i8p_ext(dl.default_address_space)], bx.type_void());
...
fn codegen_gnu_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let lpad_ty = bx.type_struct(&[bx.type_i8p_ext(dl.globals_address_space), bx.type_i32()], false);
...
fn codegen_emcc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        // TODO: Get the correct address space.
        let lpad_ty = bx.type_struct(&[bx.type_i8p_ext(dl.default_address_space), bx.type_i32()], false);
        ...
        // TODO: Get the correct address space.
        let catch_data_type = bx.type_struct(&[bx.type_i8p_ext(dl.default_address_space), bx.type_bool()], false);
    ...
    if name == sym::simd_select_bitmask {
        ...
        let mask = match mask_ty.kind() {
                        ...
                        == Some(expected_bytes) =>
            {
                ...
                let ptr = bx.pointercast(place.llval, bx.cx.type_ptr_to_ext(int_ty, dl.alloca_address_space));
    ...
    if name == sym::simd_bitmask {
        ...
        let (i_xn, in_elem_bitwidth) = match in_elem.kind() {
            ty::Int(i) => (
                ...
                i.bit_width().unwrap_or_else(|| bx.data_layout().ptr_layout(None).idx_size.bits()),
            ),
            ty::Uint(i) => (
                args[0].immediate(),
                // TODO: Stop using pointer size as an equivalent to machine word size?
        ...
        match ret_ty.kind() {
                        ...
                        == Some(expected_bytes) =>
            {
                ...
                let ptr = bx.pointercast(ptr, bx.cx.type_ptr_to_ext(array_ty, dl.alloca_address_space));
...
unsupported {} from `{}` with element `{}` of size `{}` to `{}`"#,
    ...
    if name == sym::simd_saturating_add || name == sym::simd_saturating_sub {
        ...
        // TODO: Stop using pointer size as an equivalent to machine word size?
        let ptr_bits = bx.tcx().data_layout.ptr_layout(None).idx_size.bits() as _;
