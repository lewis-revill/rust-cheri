...
impl<'ll, 'tcx> ConstMethods<'tcx> for CodegenCx<'ll, 'tcx> {
    ...
    fn const_usize(&self, i: u64) -> &'ll Value {
        ...
        let bit_size = self.data_layout().ptr_layout(None).idx_size.bits();
        if bit_size < 64 {
    ...
    fn const_str(&self, s: &str) -> (&'ll Value, &'ll Value) {
        ...
        let cs = consts::ptrcast(
            str_global,
            self.type_ptr_to_ext(self.layout_of(self.tcx.types.str_).llvm_type(self), self.tcx.data_layout.globals_address_space),
        );
