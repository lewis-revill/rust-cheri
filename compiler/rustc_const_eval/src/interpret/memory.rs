    ...
    pub fn get_alloc_info(&self, id: AllocId) -> (Size, Align, AllocKind) {
        ...
        match self.tcx.try_get_global_alloc(id) {
            ...
            Some(GlobalAlloc::VTable(..)) => {
                ...
                return (Size::ZERO, dl.ptr_layout(Some(dl.instruction_address_space)).align.abi, AllocKind::VTable);
            }
...
impl<'tcx, 'a, Prov: Provenance, Extra> AllocRefMut<'a, 'tcx, Prov, Extra> {
    ...
    pub fn write_ptr_sized(&mut self, offset: Size, val: Scalar<Prov>) -> InterpResult<'tcx> {
        ...
        self.write_scalar(alloc_range(offset, self.tcx.data_layout().ptr_layout(None).ty_size, self.tcx.data_layout().ptr_layout(None).idx_size), val)
    }
...
impl<'tcx, 'a, Prov: Provenance, Extra> AllocRef<'a, 'tcx, Prov, Extra> {
    ...
    pub fn read_pointer(&self, offset: Size) -> InterpResult<'tcx, Scalar<Prov>> {
        // TODO: More complexity needed here. idx_size vs ty_size.
        self.read_scalar(
            alloc_range(offset, self.tcx.data_layout().ptr_layout(None).ty_size, self.tcx.data_layout().ptr_layout(None).idx_size),
...
impl<'mir, 'tcx: 'mir, M: Machine<'mir, 'tcx>> InterpCx<'mir, 'tcx, M> {
    ...
    pub fn mem_copy_repeatedly(
        ...
        if src_alloc.range_has_provenance(&tcx, alloc_range(src_range.start, Size::ZERO, Size::ZERO)) {
            throw_unsup!(PartialPointerCopy(Pointer::new(src_alloc_id, src_range.start)));
        }
        if src_alloc.range_has_provenance(&tcx, alloc_range(src_range.end(), Size::ZERO, Size::ZERO)) {
