...
use rustc_middle::ty::{ConstInt, Ty, ScalarInt};
...
impl<Prov: Provenance> Immediate<Prov> {
    ...
    pub fn to_scalar_with_layout<'tcx>(self, layout: TyAndLayout<'tcx>) -> Scalar<Prov> {
        match self {
            Immediate::Scalar(Scalar::Int(int)) => Scalar::Int(ScalarInt::try_from_uint_r(int.assert_bits(int.range()), layout.transmute_size, layout.size).unwrap()),
            Immediate::Scalar(val) => val,
            Immediate::ScalarPair(..) => bug!("Got a scalar pair where a scalar was expected"),
            Immediate::Uninit => bug!("Got uninit where a scalar was expected"),
        }
